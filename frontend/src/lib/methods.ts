...
export const METHODS: Record<string, MethodsChunk> = {
  ...
  missing_data_tradeoffs: {
    title: 'Choosing between the imputation strategies',
    text: 'Median imputation is simple, but it ignores the correlation structure and shrinks variance. Iterative PCA (regularized, the missMDA imputePCA method; Josse & Husson 2016) is better when variables are correlated, but it can be biased when there are many gaps — on this app\'s own test it recovers punched-out iris values with about half the error of the median, though it is marginally worse on noise (uncorrelated variables). Complete cases keeps the covariance honest but reduces n and can bias the sample when missingness is not random (Little & Rubin, Statistical Analysis with Missing Data). All three are single imputation, so none of them carry the uncertainty of the filled values into what follows; with substantial missingness, run more than one and compare — divergent results mean the missing-data handling is driving the conclusion.',
  },
  statistical_tests: {
    title: 'The built-in statistical tests',
    text: 'Scatter Lab\'s Analyze tools run five tests, all computed in the browser. Welch\'s t (Welch 1947) compares two group means without assuming equal variances — the recommended default over Student\'s t (Delacre, Lakens & Leys 2017) — reported with Welch–Satterthwaite df, an exact two-sided p, a 95% CI on the mean difference, and Cohen\'s d (pooled sample sd; benchmarks ~0.2 small / 0.5 medium / 0.8 large, Cohen 1988). Mann–Whitney U (Mann & Whitney 1947) is its rank-based sibling: it tests whether one group\'s values tend to exceed the other\'s, needs no normality, and is reported with the tie-corrected normal approximation and the rank-biserial correlation (1 = complete separation). Kruskal–Wallis H (Kruskal & Wallis 1952) extends that comparison to 3+ groups (chi-square approximation, tie-corrected) with epsilon-squared as effect size; a significant H says "not all groups are alike", not which ones differ. The two-sample Kolmogorov–Smirnov test (Kolmogorov 1933; Smirnov 1948) compares entire distributions via the maximum ECDF gap D — it can detect differences in spread or shape that mean-based tests miss, and pairs naturally with the ECDF chart. The chi-square test of independence with Cramér\'s V (Cramér 1946) relates two categorical variables; its approximation degrades when expected cell counts fall below 5 (the classic rule from Cochran 1954), which the result flags. Reporting policy: exact p-values with CI and effect size, no significance stars — a p-value measures compatibility with "no difference", not importance; the effect size does that (Wasserstein & Lazar 2016). These are single tests on data you chose to look at: run many comparisons and some will be "significant" by chance, so treat exploratory p-values as leads, not conclusions.',
  },
  distribution_charts: {
    title: 'Reading distribution charts (ECDF, histogram, box, violin, Q–Q)',
    text: 'An ECDF plots, for each value x, the share of observations at or below x — every point of the sample, no binning decisions, and group comparisons read as horizontal gaps (the largest such gap is the KS statistic D). Histograms are more familiar but bin-dependent: this app uses shared bin edges across groups (per-group binning makes overlaid histograms incomparable) and Sturges\' rule when bins are unspecified. Box plots show the median, quartile box, whiskers to the furthest point within 1.5·IQR of the box (Tukey 1977), and outliers beyond that as points — "outlier" here is a plotting convention, not a judgment. Violins replace the box with a kernel density estimate (Gaussian kernel, Silverman\'s bandwidth, Silverman 1986; Hintze & Nelson 1998), which shows multimodality a box hides but smooths by construction — a lump needs enough n to be believed. A normal Q–Q plot ranks the sample against theoretical normal quantiles: points on the reference line (drawn through the quartiles) indicate approximate normality; S-curves indicate skew, and ends peeling away indicate heavy or light tails (Wilk & Gnanadesikan 1968).',
  },
};
